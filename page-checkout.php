<?php
/**
 * Template Name: Оформлення замовлення
 *
 * @package Womazing
 */
...
<div class="page-hero">
  ...
  <div class="page-hero__inner">
    <h1 class="page-hero__title"><?php esc_html_e( 'Оформлення замовлення', 'womazing' ); ?></h1>
    <nav class="breadcrumbs">
      <a href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'Головна', 'womazing' ); ?></a>
      <span>—</span>
      <span><?php esc_html_e( 'Оформлення замовлення', 'womazing' ); ?></span>
    </nav>
  </div>
...
(function() {
  function initFloatingLabels() {
    var rows = document.querySelectorAll('.checkout-wrap .form-row');
    rows.forEach(function(row) {
      var input = row.querySelector('input:not([type="checkbox"]):not([type="radio"]), select, textarea');
      if (!input) return;
      function update() {
  ...
  document.body.addEventListener('updated_checkout', initFloatingLabels);
})();
</script>
